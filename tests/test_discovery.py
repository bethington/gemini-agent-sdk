"""Tests for gemini_cli_sdk.discovery — binary location."""
...
from gemini_cli_sdk.discovery import find_gemini_binary
...
def test_found_on_path():
    with patch("gemini_cli_sdk.discovery.shutil.which", return_value="/usr/bin/gemini"):
        result = find_gemini_binary()
        assert result == "/usr/bin/gemini"


def test_windows_fallback(tmp_path, monkeypatch):
    monkeypatch.setattr("gemini_cli_sdk.discovery.shutil.which", lambda _: None)
    monkeypatch.setattr("gemini_cli_sdk.discovery.sys.platform", "win32")
...
def test_not_found_raises():
    with patch("gemini_cli_sdk.discovery.shutil.which", return_value=None):
        with patch("gemini_cli_sdk.discovery.os.path.isfile", return_value=False):
